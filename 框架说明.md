...
配置.NET6.0 运行时
具备的基础设施
多数据库支持 MySQL、SQLServer、Oracle、Sqlite、PostgreSql
swagger分组
日志面板地址：导航到 /logdashboard
CodeFisrt数据迁移
找到项目中的 Shipeng.EntityFrameworkCore 层
在程序包管理控制台执行 Add-Migration Init 生成迁移
执行 Update-Database Init 还原数据库
项目结构(以及层依赖说明)
