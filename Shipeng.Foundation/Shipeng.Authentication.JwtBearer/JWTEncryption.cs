...
namespace Shipeng.DataEncryption
{
    /// <summary>
    /// JWT 加解密
    /// </summary>
    public class JWTEncryption
    {
        ...
        public static (bool IsValid, JsonWebToken Token, TokenValidationResult validationResult) Validate(string accessToken)
        {
            ...
            try
            {
                var tokenValidationResult = tokenHandler.ValidateTokenAsync(accessToken, tokenValidationParameters).GetAwaiter().GetResult();
        ...
        {
            var jwtSettings= FrameworkApp.GetMethod("GetOptions").MakeGenericMethod(typeof(JWTSettingsOptions)).Invoke(null, new object[] { null }) as JWTSettingsOptions ?? SetDefaultJwtSettings(new JWTSettingsOptions());
            if (string.IsNullOrWhiteSpace(jwtSettings.IssuerSigningKey) &&
                 !string.IsNullOrWhiteSpace(jwtSettings.IssuerSigningKeyText))
            {
                //重新生成签名秘钥
                byte[] key = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(jwtSettings.IssuerSigningKeyText)).Key;
                jwtSettings.IssuerSigningKey = Convert.ToBase64String(key); //数组byte转化string
            }
            return jwtSettings;
        }
        ...
        internal static JWTSettingsOptions SetDefaultJwtSettings(JWTSettingsOptions options)
        {
            ...
            if (options.ValidateIssuerSigningKey == true)
            {
                ...
                !string.IsNullOrWhiteSpace(options.IssuerSigningKeyText))
                {
                    //重新生成签名秘钥
                    byte[] key = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(options.IssuerSigningKeyText)).Key;
                    options.IssuerSigningKey = Convert.ToBase64String(key); //数组byte转化string
                }
            }
        ...
        internal static Assembly GetFrameworkContext(Assembly callAssembly)
        {
            ...
            var furionAssemblyName = callAssembly.GetReferencedAssemblies()
                                                       .FirstOrDefault(u => u.Name == "Trace")
                                                       ?? throw new InvalidOperationException("No `Trace` assembly installed in the current project was detected.");

            // 加载 Trace 程序集
            var furionAssembly = AssemblyLoadContext.Default.LoadFromAssemblyName(furionAssemblyName);

            // 获取 Furion.App 静态类
            FrameworkApp = furionAssembly.GetType("Shipeng.App");
