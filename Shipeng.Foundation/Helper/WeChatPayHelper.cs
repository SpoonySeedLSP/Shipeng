...
namespace Shipeng.Util
{
    public class WeChatPayHelper
    {
        ...
        private static WxPayData UnifiedOrder(WxPayData inputObj)
        {
            ...
            string resData = WeChatHttpClientHelper.PostXml(url, xml);
        ...
        private static WxPayData Refund(WxPayData inputObj, string certurl, string MchId)
        {
            ...
            string xml = inputObj.ToXml();
            DateTime start = DateTime.Now;
            X509Certificate2 cert = X509CertificateLoader.LoadPkcs12FromFile(certurl, MchId, X509KeyStorageFlags.MachineKeySet);
            string resData = WeChatHttpClientHelper.PostXml(url, xml, cert);
