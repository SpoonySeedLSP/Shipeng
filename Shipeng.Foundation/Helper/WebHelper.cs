...
namespace Shipeng.Util
{
    public class WebHelper
    {
        ...
        /// </summary>
        /// <remarks>
        /// WebHelper 中的公网 IP、IP 归属地查询属于低频通用工具方法，但仍不应该每次调用都创建
        /// HttpClient。复用实例可以共享连接池，减少端口占用和 TCP/TLS 建连开销。
        /// </remarks>
        private static readonly HttpClient NetworkQueryClient = new()
        {
        ...
        public static string GetWanIp()
        {
            ...
            try
            {
                ...
                string html = "";
                var reponse = NetworkQueryClient.GetAsync(url).GetAwaiter().GetResult();
        ...
        private static string GetIpLocationFromPCOnline(string ipAddress)
        {
            ...
            try
            {
                ...
                var response = NetworkQueryClient.GetAsync(URL).GetAwaiter().GetResult();
