    ...
    public class RabbitMqHelper : IDisposable
    {
            ...
            where T : class
        {
            ...
            for (int i = 0; i < consumerCount; i++)
            {
                ...
                //注册事件
                if (registered != null)
                    consumer.RegisteredAsync += (sender, ea) => { registered(sender, ea); return Task.CompletedTask; };

                //取消注册事件
                if (unregistered != null)
                    consumer.UnregisteredAsync += (sender, ea) => { unregistered(sender, ea); return Task.CompletedTask; };

                //关闭事件
                if (shutdown != null)
                    consumer.ShutdownAsync += (sender, ea) => { shutdown(sender, ea); return Task.CompletedTask; };
