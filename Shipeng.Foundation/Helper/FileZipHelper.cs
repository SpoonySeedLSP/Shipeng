...
namespace Shipeng.Util
{
    ...
    public class FileZipHelper
    {
        ...
        public static void Zip(string[] files, string ZipedFileName, string Password)
        {
            ...
            using ICSharpCode.SharpZipLib.Zip.ZipOutputStream s = new ICSharpCode.SharpZipLib.Zip.ZipOutputStream(File.Create(ZipedFileName));
            s.SetLevel(6);
            if (!string.IsNullOrWhiteSpace(Password)) s.Password = Password.Trim();
        ...
        private static void ZipFileDictory(string[] files, ICSharpCode.SharpZipLib.Zip.ZipOutputStream s)
        {
            ...
            // 创建当前文件夹。加上 "/" 才会被 Zip 客户端识别为目录。
            var rootEntry = new ICSharpCode.SharpZipLib.Zip.ZipEntry("/");
            s.PutNextEntry(rootEntry);
            s.CloseEntry();
    ...
    public class DotNetZipHelper
    {
        ...
        {
            var fileList = directory.EnumerateFiles("*", SearchOption.AllDirectories);
            using var zipArchive = new ZipArchive(stream, ZipArchiveMode.Create);
            foreach (var file in fileList)
            {
                string relativePath = Path.GetRelativePath(directory.FullName, file.FullName);
                var zipArchiveEntry = zipArchive.CreateEntry(relativePath, System.IO.Compression.CompressionLevel.Fastest);
                await using var entryStream = zipArchiveEntry.Open();
                await using var toZipStream = file.OpenRead();
        ...
        public static void CompressionFile(string filePath, string zipPath, List<string> filterExtenList = null)
        {
            try
            {
                using (var zip = File.Create(zipPath))
                {
                    ...
                    using (var zipWriter = WriterFactory.OpenWriter(zip, ArchiveType.Zip, option))
                    {
                        if (Directory.Exists(filePath))
                        {
                            //添加文件夹
        ...
        public static void DeCompressionFile(string zipPath, string dirPath, string password = "")
        {
            ...
            try
            {
                using (Stream stream = File.OpenRead(zipPath))
                {
                    ...
                    var reader = ReaderFactory.OpenReader(stream, option);
