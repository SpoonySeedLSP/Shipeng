...
namespace Shipeng.Util
{
    ...
    public class StringHelper
    {
        ...
        private const string SQL_FILTER_STRINGS = "=,',:, or , and ,select,update,insert,delete,declare,exec,drop,create,%,--";
        ...
        /// <returns> </returns>
        public static string SqlFilter(string filterStr)
        {
            if (!string.IsNullOrEmpty(filterStr))
            {
                string[] SQL = SQL_FILTER_STRINGS.Split(',');
        ...
        /// <returns> 全拼 </returns>
        public static string WP_GetPinyin(string str)
        {
        ...
        /// <returns> </returns>
        public static string W_GJC(string key)
        {
            Regex reg = new Regex(@"[^\u4e00-\u9fa5]+");//符合
                                                        // Regex reg = new Regex(@"[\u4e00-\u9fa5]");//失败中文
            key = reg.Replace(key, ",");

            return string.Join(",", key.Split(',').ToList().ToDistinctList().ToArray());
        }
        ...
        /// <param name="str"> 汉字 </param>
        /// <returns> 首字母 </returns>
        public static string WP_GetFirstPinyin(string str)
        {
            string r = string.Empty;
            if (!str.IsNullOrEmpty())
            {
        ...
        /// <param name="str"> 汉字 </param>
        /// <returns> 全拼 </returns>
        public static string WP_GetPinying(string str)
        {
            string r = string.Empty;
            if (!str.IsNullOrEmpty())
            {
        ...
        /// <param name="str"> 汉字 </param>
        /// <returns> 首字母 </returns>
        public static string WP_GetFirstPinying(string str)
        {
            string r = string.Empty;
            if (!str.IsNullOrEmpty())
            {
        ...
        /// <returns> </returns>
        public static string WP_Soso(string str)
        {
            str = str + "," + WP_GetPinying(str);
            List<string> listso = str.Replace(" ", "").Split(',').ToList();
            listso = listso.Where(s => !string.IsNullOrEmpty(s) && s.Length > 1).Distinct().ToList();
            listso = listso.OrderByDescending(x => x).ToList();
            str = string.Join(",", listso.ToArray());
            return str;
        }
        ...
        public static List<string> Splitstring(string str, int separatorCharNum)
        {
        ...
        public static string WP_HtmlTxt(string TxxXT)
        {
            if (!TxxXT.IsNullOrEmpty())
            {
                TxxXT = Regex.Replace(TxxXT, "<[P|p][^>]*>", "<p>");
                TxxXT = Regex.Replace(TxxXT, "</[P|p][^>]*>", "</p>");//P
                TxxXT = Regex.Replace(TxxXT, "<[P|p][R|r][E|e][^>]*>", "<p>");
                TxxXT = Regex.Replace(TxxXT, "</[P|p][R|r][E|e][^>]*>", "</p>");//PRE
                TxxXT = Regex.Replace(TxxXT, "<[d|D][i|I][v|V][^>]*>", "");
                TxxXT = Regex.Replace(TxxXT, "</[d|D][i|I][v|V]>", "");

                TxxXT = Regex.Replace(TxxXT, "<[f][o][n][t][^>]*>", "");
                TxxXT = Regex.Replace(TxxXT, "<?[x][m][l][^>]*/>", "");
                TxxXT = Regex.Replace(TxxXT, "</[f][o][n][t][^>]*>", "");
                TxxXT = Regex.Replace(TxxXT, "<[s][p][a][n][^>]*>", "<span>");
                TxxXT = Regex.Replace(TxxXT, "<[s][e][c][t][i][o][n][^>]*>", "<section>");//section
                TxxXT = Regex.Replace(TxxXT, "<[s][t][r][o][n][g][^>]*>", "<strong>");//strong
                TxxXT = Regex.Replace(TxxXT, "<[u][l][^>]*>", "");
                TxxXT = Regex.Replace(TxxXT, "</[u][l][^>]*>", "");
                TxxXT = Regex.Replace(TxxXT, "<[l][i][^>]*>", "");
                TxxXT = Regex.Replace(TxxXT, "</[l][i][^>]*>", "");
                TxxXT = Regex.Replace(TxxXT, "<[b][r][^>]*>", "<br>");
                TxxXT = Regex.Replace(TxxXT, "<[h][2][^>]*>", "<h2>");

                TxxXT = Regex.Replace(TxxXT, @"style\s*=(['""\s]?)[^'""]*?\1", "");

                TxxXT = TxxXT.Replace("&nbsp;", "");
                TxxXT = TxxXT.Replace("&rdquo;", "");
                TxxXT = TxxXT.Replace("&ldquo;", "");
                TxxXT = TxxXT.Replace("&mdash;", "-");
                TxxXT = TxxXT.Replace("&hellip;", "");
                TxxXT = TxxXT.Replace("&deg;", "");
                TxxXT = TxxXT.Replace("&prime;", "");
                TxxXT = TxxXT.Replace("&deg;", "");
                TxxXT = TxxXT.Replace("&deg;", "");

                TxxXT = TxxXT.Replace("<span>，</span>", "");
                TxxXT = TxxXT.Replace("<span>。</span>", "");
                TxxXT = TxxXT.Replace("<span>、</span>", "");

                TxxXT = Compress(TxxXT);

                TxxXT = TxxXT.Replace("border='0'", "");
                TxxXT = TxxXT.Replace("align=middle", "");
                TxxXT = TxxXT.Replace("border=0", "");
                TxxXT = TxxXT.Replace("<span></span>", "");
                TxxXT = TxxXT.Replace("<center></center>", "");
                TxxXT = TxxXT.Replace("<section></section>", "");
                TxxXT = TxxXT.Replace("<p></p>", "");
                TxxXT = TxxXT.Replace("<span></span>", "");
                TxxXT = TxxXT.Replace("<center></center>", "");
                TxxXT = TxxXT.Replace("<strong></strong>", "");
                if (TxxXT.Length < 1)
                {
                    TxxXT = "<p></p>";
                }
        ...
        /// <param name="regexs"> 正则 </param>
        /// <returns> </returns>
        public static List<string> WP_RegexArray(string str, string regexs)
        {
            List<string> myArrList = new List<string>();
            Regex r = new Regex(regexs);
            foreach (Match m in r.Matches(str))
            {
                myArrList.Add(m.Groups[0].Value);
            }
            return myArrList.Distinct().ToList();
        }
        ...
        /// <returns> </returns>
        public string WP_CompressString(string text)
        {
            try
            {
        ...
        /// <returns> </returns>
        public string WP_DecompressString(string compressedText)
        {
            try
            {
            ...
            {
                return StringDeCompress(compressedText);
            }
        }

        #region 老方解压缩字符串

        public string StringDeCompress(string strCompressed)
        {
            try
            {
                using (Stream s = new GZipStream(new MemoryStream(System.Convert.FromBase64String(strCompressed)), CompressionMode.Decompress))
                {
                    StringBuilder strUncompressed = new StringBuilder();
                    byte[] dataWrite = new byte[30720];
                    int totalLength = 0;
                    int size = 0;
                    while (true)
                    {
                        size = s.Read(dataWrite, 0, dataWrite.Length);
                        if (size > 0)
                        {
                            totalLength += size;
                    ...
                    s.Close();
                    s.Dispose();
                    strCompressed = strUncompressed.ToString();
                }
        ...
        /// <param name="rep"> 替换符合* </param>
        /// <returns> </returns>
        public static string WP_SensitiveWordFiltering(string text, List<string> Words, char rep = '*')
        {
            try
            {
        ...
        /// <param name="text"> 文本 </param>
        /// <param name="Words"> 关键词 </param>
        /// <param name="split"> 关键词分隔符 </param>
        /// <param name="rep"> 替换符合* </param>
        public string WP_SensitiveWordFiltering(string text, string Words, char split = ',', char rep = '*')
        {
            try
            {
                //string illegalWords = "做爱|傻逼|三级片|傻x|操你妈";//可以从数据库或者txt文本中读取非法关键词(缓存什么也可以加上)

                StringSearch iwords = new StringSearch();
                iwords.SetKeywords(Words.Split(split));
                text = iwords.Replace(text, rep);
            }
            catch //(Exception ex)
            {
        ...
        /// </summary>
        public static string _QueryStringKey = "19880911"; //URL传输参数加密Key这个key可以自己设置支持8位这个东西很重要的
        ...
        /// <returns> </returns>
        public static string WP_Encrypt(string QueryString)
        {
        ...
        /// <param name="QueryString"> </param>
        /// <returns> </returns>
        public static string WP_Decrypt(string QueryString)
        {
        ...
        /// <returns> </returns>
        public static string WP_Encrypt(string QueryString, string Key)
        {
            return Encrypt(QueryString, Key);
        }
        ...
        /// <param name="Key"> 密匙 </param>
        /// <returns> </returns>
        public static string WP_Decrypt(string QueryString, string Key)
        {
            return Decrypt(QueryString, Key);
        }
        ...
        private static string Encrypt(string pToEncrypt, string sKey)
        {
            try
            {
                using DES des = DES.Create(); //把字符串放到byte数组中
        ...
        public static int GetCountByAppointString(string sourceString, string appointString, string type)
        {
            ...
            switch (type)
            {
                case "split": int_count = sourceString.Split(new string[] { appointString }, StringSplitOptions.None).Count() - 1; break;
                case "replace":
                    sourceString = sourceString.Replace(appointString, string.Empty);
                    int_count = (int_SstringLength - sourceString.Length) / int_AstringLength; break;
        ...
        /// <returns></returns>
        public static string StringReplace(string Text, List<string> list)
        {
            if (!Text.IsNullOrEmpty())
            {
                if (Text.Contains("${"))
