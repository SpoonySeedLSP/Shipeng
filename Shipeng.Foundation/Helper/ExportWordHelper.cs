...
namespace Shipeng.Util.Helper
{
     ...
     * Description：2021年2月17日
     */
    public class ExportWordHelper
    {
        private static ExportWordHelper _exportHelper;

        public static ExportWordHelper _
        {
            get => _exportHelper ?? (_exportHelper = new ExportWordHelper());
        ...
        /// <param name="isStatement">是否在同一段落创建第二个文本对象（解决同一段落里面需要填充两个或者多个文本值的情况，
        /// 多个文本需要自己拓展，现在最多支持两个）</param>
        ...
        /// <returns></returns>
        public XWPFParagraph ParagraphInstanceSetting(XWPFDocument document, string fillContent, bool isBold, int fontSize,
            string fontFamily, ParagraphAlignment paragraphAlign, bool isStatement = false, string secondFillContent = "",
            string fontColor = "000000", bool isItalic = false)
        {
        ...
        public XWPFParagraph SetTableParagraphInstanceSetting(XWPFDocument document, XWPFTable table, string fillContent,
            ParagraphAlignment paragraphAlign, int textPosition = 24, bool isBold = false, int fontSize = 10,
            string fontColor = "000000", bool isItalic = false)
        {
            var para = new CT_P();
            //设置单元格文本对齐
