...
using Shipeng.Templates.Extensions;
...
namespace Shipeng.UnifyResult
{
    ...
    public static class UnifyContext
    {
        ...
        public static void SetResponseStatusCodes(HttpContext context, int statusCode, UnifyResultSettingsOptions unifyResultSettings)
        {
            if (context == null || unifyResultSettings == null)
                return;

            // 响应已经开始输出后，不能再修改 StatusCode，否则会抛：
            ...
            if (unifyResultSettings.AdaptStatusCodes != null && unifyResultSettings.AdaptStatusCodes.Length > 0)
            {
                var adaptStatusCode = unifyResultSettings.AdaptStatusCodes
                    .FirstOrDefault(u => u != null && u.Length > 1 && u[0] == statusCode);
            ...
            // 否则只有里面的才设置为 200
            if (unifyResultSettings.Return200StatusCodes.Contains(statusCode))
            {
                context.Response.StatusCode = 200;
            }
        }
