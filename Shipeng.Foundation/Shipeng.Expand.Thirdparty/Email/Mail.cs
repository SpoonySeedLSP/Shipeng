...
using Shipeng.Expand.Thirdparty.Email.Model;
...
namespace Shipeng.Expand.Thirdparty.Email
{
    /// <summary>
    /// 基于MailKit的邮件帮助类
    /// 版 本：V1.0.0
    ...
    public class Mail
    {
        private static readonly string mailFilePath = Path.Combine(AppContext.BaseDirectory, "EmailFiles") + Path.DirectorySeparatorChar;
        ...
        public static void Send(MailAccount mailConfig, MailModel mailModel)
        {
            ...
            if (!string.IsNullOrEmpty(mailModel.To))
            {
                ...
                {
                    toAddress.Add(new MailboxAddress("云南通科科技有限公司",item));
                }
            ...
            if (!string.IsNullOrEmpty(mailModel.CC))
            {
                ...
                {
                    ccAddress.Add(new MailboxAddress("云南通科科技有限公司",item));
                }
            ...
            if (!string.IsNullOrEmpty(mailModel.Bcc))
            {
                ...
                {
                    bccAddress.Add(new MailboxAddress("云南通科科技有限公司",item));
                }
        ...
        private static List<MailFile> GetEmailAttachments(IEnumerable<MimeEntity> attachments, string messageId)
        {
            ...
            Directory.CreateDirectory(mailFilePath);
            Directory.CreateDirectory(mailFilePath);
            foreach (var attachment in attachments)
            {
                if (attachment.IsAttachment)
                {
                    ...
                    var fileId = messageId + "_" + fileName;
                    ...
                    resultList.Add(new MailFile { FileId = fileId, FileName = fileName, FileSize = FormatFileSize(mailFileInfo.Length) });
                }
            }
            return resultList;
        }
