    ...
    public class FlowinstanceService : DataFilterService<FlowinstanceEntity>, IDenpendency
    {
        ...
        private string GetNextMakers(FlowRuntime wfruntime, NodeDesignateEntity request = null)
        {
            ...
            { //如果是运行时指定角色
                ...
                makerList = JsonExtention.ArrayToString(users.Distinct().ToList(), makerList);
            }
            ...
            {  //如果是运行时指定用户
                ...
                makerList = JsonExtention.ArrayToString(request.NodeDesignates, makerList);
            }
            else
            {
        ...
        private string GetNodeMarkers(FlowNode node)
        {
            ...
            if (node.setInfo != null)
            {
                ...
                {
                    makerList = JsonExtention.ArrayToString(node.setInfo.NodeDesignateData.users, makerList);
                }
                else if (node.setInfo.NodeDesignate == Setinfo.SPECIAL_ROLE)  //指定角色
                {
                    ...
                    makerList = JsonExtention.ArrayToString(users, makerList);
                }
        ...
        public async Task CreateInstance(FlowinstanceEntity entity)
        {
            ...
            Dictionary<string, string> dic = JsonExtention.ToObject<Dictionary<string, string>>(entity.F_FrmData);
