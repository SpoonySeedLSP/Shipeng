...
{
    /// <summary>
    /// 父子数据相互映射类 注：父子数据都必须唯一,即所有父键唯一,所有子键唯一,唯一的子键能确定对应的父键,唯一的父键能确定其拥有的子键集合,
    /// </summary>
    public class ParentChildrenMapping
    {
        ...
        public void RemoveChild(string parentId, string childId)
        {
            ...
            ConcurrentBag<string> children = _pToC[parentId];
            if (children == null)
            {
        ...
        public void RemoveParent(string parentId)
        {
            ...
            if (children != null)
            {
                IEnumerator<string> enumerator = children.GetEnumerator();
                do
                {
