...
namespace Shipeng.Util.DataBase
{
    public static partial class WaterQuery
    {
        ...
        public static Expression ConvertList<T>(List<FilterList> filterList, ParameterExpression param)
        {
            ...
            Expression result = ConvertFilters<T>(filterList[0].Filters.ToObject<Filter[]>(), param, filterList[0].Operation);
            foreach (var item in filterList.Skip(1))
            {
                ...
                var gresult = ConvertFilters<T>(item.Filters.ToObject<Filter[]>(), param, item.Operation);
