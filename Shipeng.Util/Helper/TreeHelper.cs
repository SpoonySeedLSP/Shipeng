namespace Shipeng.Util
{
    ...
    public class TreeHelper
    {
        ...
        public static List<T> BuildTree<T>(List<T> allNodes) where T : TreeModel, new()
        {
            ...
            List<T> rootNodes = allNodes.Where(x => x.ParentId == "0" || x.ParentId.IsNullOrEmpty()).ToList();
            resData = rootNodes;
        ...
        public static List<T> GetChildren<T>(List<T> allNodes, T parentNode, bool includeSelf) where T : TreeModel
        {
            ...
            {
                List<T> children = _allNodes.Where(x => x.ParentId == _parentNode.Id).ToList();
        ...
        {
            Type type = typeof(T);
            List<System.Reflection.PropertyInfo> properties = type.GetProperties().ToList();
            List<object> resData = new List<object>();
            List<T> children = nodes.Where(x => x.ParentId == parentNode.Id).ToList();
            children.ForEach(aChildren =>
            {
                ...
                {
                    object value = aProperty.GetValue(aChildren, null);
