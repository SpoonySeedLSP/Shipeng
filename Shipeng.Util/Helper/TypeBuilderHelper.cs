...
namespace Shipeng.Util
{
    ...
    public static class TypeBuilderHelper
    {
        ...
        {
            Type attributeType = attributeConfig.Attribute;
            ConstructorInfo attributeConstructor = attributeType.GetConstructor(attributeConfig.ConstructorArgs.Select(x => x.GetType()).ToArray());
            List<(PropertyInfo PropertyInfo, object Value)> properties = new List<(PropertyInfo, object)>();
            List<PropertyInfo> allProperties = attributeType.GetProperties().ToList();
            attributeConfig.Properties.ForEach(aProperty =>
            {
                PropertyInfo theProperty = allProperties.Where(x => x.Name == aProperty.PropertyName).FirstOrDefault();
                if (theProperty != null)
                {
